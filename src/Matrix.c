...
ErrorCode matrix_copy(PMatrix* result, CPMatrix source){
    ...
    for (uint32_t i = 0; i < source->heigth; ++i) {
...
void matrix_destroy(PMatrix matrix) {
    if (matrix != NULL) {
        ...
        for (uint32_t x = 0; x < matrix->heigth; ++x){
            free(matrix->arr[x]);
        }
...
ErrorCode matrix_add(PMatrix* result, CPMatrix lhs, CPMatrix rhs) {
    ...
    for (uint32_t i = 0; i < lhs->heigth; ++i) {
...
ErrorCode matrix_multiplyMatrices(PMatrix* result, CPMatrix lhs, CPMatrix rhs){
    ...
    if (result == NULL) {
        ErrorCode e = matrix_create(result,lhs->heigth,lhs->width);
        if (!error_isSuccess(e)) {
...
ErrorCode matrix_multiplyWithScalar(PMatrix matrix, double scalar){
    ...
    for (uint32_t i = 0; i < matrix->heigth; ++i) {
